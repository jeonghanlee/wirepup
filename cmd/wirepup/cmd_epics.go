...
func runEPICSFind(ctx context.Context, e *env, args []string) int {
	...
	g.register(fs)
	a.register(fs)
	fs.BoolVar(&activeSearch, "active", false, "send a CA search (ACTIVE: transmits one datagram per destination)")
	fs.StringVar(&to, "to", "", "comma-separated search destinations host[:port] for --active (default: directed broadcasts of the interface)")
	...
	dests, err := searchDestinations(&g, to)
	if err != nil {
		fmt.Fprintf(e.stderr, "wirepup: %v\n", err)
		return exitUsage
	}
	...
	fmt.Fprintf(e.stderr, "ACTIVE: will send one CA search for %s to %s\n", pv, strings.Join(names, ", "))
	...
	res, err := active.CASearch(ctx, pv, dests, activeSearchID, true, wait)
	report.Executed = append(report.Executed, diagnose.Finding{Code: "ca-search-sent", Text: fmt.Sprintf("sent %d CA search datagram(s) for %s to %s (reply requested)", len(res.Sent), pv, strings.Join(names, ", ")), Data: map[string]string{"pv": pv, "destinations": strings.Join(names, ",")}})
	for _, r := range res.Responses {
		report.Observed = append(report.Observed, diagnose.Finding{Code: "ca-search-answer", Text: fmt.Sprintf("server %s answered for %s: TCP port %d (datagram from %s)", r.ServerIP, pv, r.TCPPort, r.From), Data: map[string]string{"server": r.ServerIP.String(), "tcp_port": fmt.Sprint(r.TCPPort), "pv": pv}})
	}
	for _, r := range res.NotFound {
		report.Observed = append(report.Observed, diagnose.Finding{Code: "ca-not-found", Text: fmt.Sprintf("%s replied not found for %s", r.From, pv)})
	}
	switch {
	case len(res.Responses) > 1:
		report.Inferred = append(report.Inferred, diagnose.Finding{Code: "ca-multiple-servers", Text: fmt.Sprintf("%d servers claim %s; a client would connect to whichever answered first", len(res.Responses), pv)})
	case len(res.Responses) == 0:
		report.Inferred = append(report.Inferred, diagnose.Finding{Code: "ca-no-answer", Text: fmt.Sprintf("no server answered for %s within %s from the destinations tried; the PV may still exist on a server not reached by these searches", pv, wait)})
	}
...
func passiveFindReport(table *device.Table, pv, iface string) diagnose.Report {
	...
	for _, srv := range table.CAServers() {
		for _, name := range srv.PVs {
			if name == pv {
				r.Inferred = append(r.Inferred, diagnose.Finding{Code: "ca-server-claims", Text: fmt.Sprintf("%s (TCP %d) claims %s", srv.Addr, srv.TCPPort, pv), Evidence: []diagnose.Ref{srv.Ref}})
			}
		}
	}
	if len(r.Observed) == 0 {
		r.Observed = append(r.Observed, diagnose.Finding{Code: "ca-nothing-seen", Text: fmt.Sprintf("no CA search or answer for %s was observed", pv)})
		r.Observed = r.Observed[:0]
		r.Inferred = append(r.Inferred, diagnose.Finding{Code: "ca-nothing-seen", Text: fmt.Sprintf("nothing about %s crossed this interface during the window; nothing can be said about the PV from passive observation alone", pv)})
	}
	return r
}

func searchDestinations(g *globalFlags, to string) ([]netip.AddrPort, error) {
	...
	for _, s := range strings.Split(to, ",") {
		...
		if err != nil {
			...
			if aerr != nil {
				return nil, fmt.Errorf("%w: --to %q: %v", errUsage, s, err)
			}
			ap = netip.AddrPortFrom(a, ca.DefaultServerPort)
		}
		out = append(out, ap)
	}
	if len(out) > 0 {
	...
	if g.iface == "" {
		return nil, fmt.Errorf("%w: --active needs --to <host[:port],...> or -i <interface> for its broadcast addresses", errUsage)
	}
	ifc, err := interfaces.ByName(g.iface)
	if err != nil {
		return nil, err
	}
	out = active.BroadcastDestinations(ifc.IPv4, ca.DefaultServerPort)
	if len(out) == 0 {
		return nil, fmt.Errorf("%w: %s has no IPv4 subnet to broadcast on; use --to", errUsage, g.iface)
	}
	return out, nil
}
